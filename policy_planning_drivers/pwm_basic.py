...
with open('results\\imagination_greedy_comparison072622.csv', 'w') as csvfile:
...
def driver(initial_state):
    ...
    with open('results\\imagination_greedy_comparison072622.csv', 'a', newline = '') as csvfile:
        w = csv.writer(csvfile, delimiter = ' ', quotechar = '|', quoting = csv.QUOTE_MINIMAL)
    ...
    for plan_percent in plan_percentages:
        ...
        for en in range(env_num):
            ...
            for trial in range(trials_num):
                ...
                max_dir = brain_imagine.take_action(nums[trial], initial_state)
                ...
                max_dir = brain_greedy.take_action(nums[trial], initial_state)
                _, reward, _, _ = env.step(max_dir)
